...
export default function AdminDashboardPage() {
  ...
  const recentActivities = [
    { time: '2 min ago', user: 'user@example.com', action: 'Opened BTC/USDT position', amount: '$5,000' },
    { time: '5 min ago', user: 'trader@example.com', action: 'Closed ETH/USDT position', amount: '$3,200' },
    ...
    { time: '25 min ago', user: 'newuser@example.com', action: 'Completed KYC verification', amount: '-' },
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-1">Welcome back, monitor your platform performance</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <Card className="p-6 hover:shadow-lg transition-shadow">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Trading Volume</h2>
          ...
          <Card className="p-6 hover:shadow-lg transition-shadow">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">User Distribution</h2>
            <div className="flex items-center justify-center h-64">
              <div className="relative w-48 h-48">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                  <circle cx="50" cy="50" r="40" fill="none" stroke="#e2e8f0" strokeWidth="20" />
                  <circle cx="50" cy="50" r="40" fill="none" stroke="#2563eb" strokeWidth="20" strokeDasharray="188.4 62.8" />
                  <circle cx="50" cy="50" r="40" fill="none" stroke="#60a5fa" strokeWidth="20" strokeDasharray="62.8 188.4" strokeDashoffset="-188.4" />
                </svg>
        ...
        <Card className="p-6 hover:shadow-lg transition-shadow">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Recent Activities</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="border-b border-slate-200 dark:border-slate-700">
                <tr>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Time</th>
              ...
              <tbody>
                {recentActivities.map((activity, index) => (
                  <tr key={index} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    <td className="py-3 px-4 text-sm text-slate-600 dark:text-slate-400">{activity.time}</td>
                    <td className="py-3 px-4 text-sm text-slate-900 dark:text-white">{activity.user}</td>
                    <td className="py-3 px-4 text-sm text-slate-700 dark:text-slate-300">{activity.action}</td>
                    <td className="py-3 px-4 text-sm font-medium text-slate-900 dark:text-white">{activity.amount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
