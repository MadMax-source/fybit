...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const getToastStyles = (type: ToastType) => {
    const styles = {
      ...
      warning: 'bg-amber-50 dark:bg-amber-900/30 border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-400',
    ...
    <ToastContext.Provider value={{ showToast }}>
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`max-w-sm px-4 py-3 rounded-lg border shadow-lg flex items-center justify-between animate-slide-in ${getToastStyles(toast.type)}`}
          >
